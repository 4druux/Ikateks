...
interface ModalProps {
  item: TranslatedServiceItem;
  onClose: () => void;
}

const ModalService = ({ item, onClose }: ModalProps) => {
    ...
    >
      ...
      >
        ...
        <div className="sticky top-9 md:top-0 z-10 flex items-center justify-between border-b border-zinc-200 bg-white px-4 md:px-6 py-2 md:py-4 shadow-sm md:shadow-none">
          <h2 className="text-2xl font-bold text-zinc-800">{item.title}</h2>
          <button
            onClick={onClose}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <ul className="space-y-2">
                {item.overviewMethodology.map((method: string) => (
                  <li key={method} className="flex items-start gap-3">
                    <CheckCircle
                      className="w-5 h-5 text-sky-600 mt-1 flex-shrink-0"
                      strokeWidth={2}
                    />
